...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      await fetchMonthly(data.user.id);
      setLoading(false);
  ...
  async function addTransaction(e) {
    ...
    const { error } = await supabase.from("transactions").insert([
      { user_id: userId, amount: n, type, note, category },
    ]);
    ...
    // (en basit: sayfayı yeniden hesaplamak yerine tekrar çağırıyoruz)
    const start = new Date();
    start.setDate(1);
    start.setHours(0, 0, 0, 0);
  ...
  return (
    <main style={{ maxWidth: 780 }}>
      <h1>Dashboard</h1>
      <p>
        Giriş yapan kullanıcı: <b>{email}</b>
      </p>
